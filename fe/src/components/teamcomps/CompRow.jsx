import { ChevronDown, Copy, Star, TrendingUp } from "lucide-react";
  ...
  return (
    <div className="flex flex-col items-center gap-1 w-16 shrink-0">
      <div className="relative">
        ...
        <div
          className={`w-12 h-12 rounded-md border-2 ${COST_BORDER[cost]} bg-slate-700 flex items-center justify-center text-[11px] font-bold text-slate-200`}
        >
          {initials}
        </div>
      </div>
...
const CompRow = ({ comp }) => {
  return (
    <div className="flex items-center gap-4 bg-slate-900/60 hover:bg-slate-800/60 border border-slate-800 rounded-lg px-4 py-3 transition-colors">
      ...
      <div className="w-56 shrink-0">
        ...
        <div className="flex flex-wrap gap-1.5 mt-1.5">
          {comp.tags.map((tag) => (
            <span
              key={tag.label}
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${TAG_STYLES[tag.type]}`}
            >
              {tag.label}
            </span>
          ))}
